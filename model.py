...
class decoder(Chain):
    def __init__(self, in_channels, out_channels, skip_channels, activation=F.average_pooling_2d):
        super(decoder, self).__init__()
        with self.init_scope():
            self.conv0 = L.Convolution2D(skip_channels + in_channels, out_channels * 4 , ksize=1)
            self.conv0 = L.Convolution2D(skip_channels + in_channels, out_channels * 4 , ksize=3, pad=1)
            # self.deconv0 = L.Deconvolution2D(out_channels, out_channels, ksize=1)
        self.activation = activation
    
    def __call__(self, x, skip):
        h = F.concat([x, skip])
        h = self.activation(self.conv0(h))
        h = pixelShuffler(h, 2)
        return h
